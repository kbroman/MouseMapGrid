...
It turns out these are in build 37 basepairs, so I need to use
[liftOver](https://genome.ucsc.edu/cgi-bin/hgLiftOver) to convert them
to build 38 basepairs.

```{r save_liu_build37_to_file}
write.table(cbind(chr=paste0("chr", liu_map$chr), pos=paste(liu_map$bp,liu_map$bp,sep="-")),
...
liu_build38[,1] <- factor(substr(liu_build38[,1], 4, nchar(liu_build38[,1])), levels=c(1:19,"X"))
liu_build38[,2] <- as.numeric(liu_build38[,2])
stopifnot(all(liu_build38[,1] == liu_map$chr))
liu_map$build38 <- liu_build38[,2]
```
